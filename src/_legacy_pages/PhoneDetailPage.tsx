...
import {
  ...
  ArrowLeft, Shield, Globe, Bluetooth, Nfc, Loader2, AlertCircle
} from "lucide-react";
import { getDevice, Device, wishlist as wishlistAPI } from "@/lib/api";
import { useAuth } from "@/contexts/AuthContext";

// â”€â”€â”€ Spec Table Row â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Spec Section â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    <div className="glass-card rounded-xl overflow-hidden mb-3">
      ...
      <div className="px-5 pb-1">
        <table className="w-full">
          <tbody>
            {rows.map((row, i) => <SpecRow key={i} label={row.label} value={row.value} />)}
          </tbody>
        </table>
      </div>
    </div>
...
// â”€â”€â”€ Build spec sections from Device full_specs_json â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function buildSpecSections(phone: Device) {
  if (phone.full_specs_json) {
    try {
      const parsed = typeof phone.full_specs_json === "string"
        ? JSON.parse(phone.full_specs_json)
        : phone.full_specs_json;
  ...
  if (phone.build) bodyRows.push({ label: "Build", value: phone.build });
  if (phone.sim) bodyRows.push({ label: "SIM", value: phone.sim });
  if (phone.water_resistance) bodyRows.push({ label: "Water Resistance", value: phone.water_resistance });
  ...
  if (phone.display_resolution) displayRows.push({ label: "Resolution", value: phone.display_resolution });
  ...
  if (phone.cpu) platformRows.push({ label: "CPU", value: phone.cpu });
  if (phone.gpu) platformRows.push({ label: "GPU", value: phone.gpu });
  if (platformRows.length) sections.push({ category: "PLATFORM", color: CAT_COLORS.PLATFORM, rows: platformRows });
  const memRows = [];
  if (phone.internal_storage) memRows.push({ label: "Internal", value: phone.internal_storage });
  if (phone.card_slot) memRows.push({ label: "Card slot", value: phone.card_slot });
  ...
  if (phone.models) miscRows.push({ label: "Models", value: phone.models });
...
export default function PhoneDetailPage() {
  ...
  useEffect(() => {
    ...
    getDevice(id)
      ...
      .catch(() => {
        setError("Phone not found. It may not be in our database yet.");
        setLoading(false);
      });
  }, [id]);
  ...
  const handleWishlist = async () => {
    ...
    try {
      if (inWishlist) {
        await wishlistAPI.remove(phone.slug);
      } else {
        await wishlistAPI.add(phone.slug);
      }
  ...
  // â”€â”€â”€ Error / Not Found â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  if (error || !phone) {
    return (
      <div className="min-h-screen page-bg">
        <Navbar />
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 px-4">
          <div className="glass-card rounded-2xl p-10 text-center max-w-md">
            ...
            <Link to="/phone-finder" className="bg-primary text-primary-foreground px-6 py-3 rounded-xl font-bold text-sm hover:bg-primary/90 transition-colors">
              Browse All Phones
            </Link>
          </div>
        </div>
    ...
    <div className="min-h-screen page-bg">
      ...
      <div className="px-4 pt-4 pb-1">
        <nav className="flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          <Link to="/phone-finder" className="hover:text-primary transition-colors">Mobiles</Link>
          <ChevronRight size={12} />
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-3">
                  ...
                  {phone.announced && (
                    <span className="text-xs text-muted-foreground py-1">{phone.announced}</span>
                  )}
                </div>
                ...
                {phone.rating && (
                  <div className="flex items-center gap-2 mb-4">
                    <div className="flex gap-0.5">
                      {[1,2,3,4,5].map(i => (
                        <Star key={i} size={14} className={i <= Math.round(parseFloat(phone.rating || "0") / 2) ? "fill-amber-400 text-amber-400" : "fill-muted text-muted"} />
                      ))}
                    </div>
                ...
                <div className="flex flex-wrap gap-3">
                  <a
                    ...
                    className="flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-sm text-white shadow-lg hover:opacity-90 transition-all bg-primary"
                  >
                  ...
                  <Link
                    to="/compare"
                    className="glass-card flex items-center gap-2 px-5 py-3 rounded-xl font-bold text-sm hover:border-primary/50 transition-all"
                  >
                  ...
                  <button
                    onClick={() => {
                      if (navigator.share) {
                        navigator.share({ title: phone.name, url: window.location.href }).catch(() => {});
                      } else {
                        navigator.clipboard.writeText(window.location.href);
                      }
                    }}
                    className="glass-card flex items-center gap-2 px-4 py-3 rounded-xl font-bold text-sm hover:border-primary/50 transition-all"
                  >
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  ...
                  {phone.network?.includes('5G') && (
                    <span className="text-xs bg-blue-500/10 text-blue-500 border border-blue-500/20 px-2.5 py-1 rounded-full font-medium">5G</span>
                  )}
                  {phone.nfc?.toLowerCase().includes('yes') && (
                    <span className="text-xs bg-purple-500/10 text-purple-500 border border-purple-500/20 px-2.5 py-1 rounded-full font-medium">NFC</span>
                  )}
                  {phone.colors && (
                    <span className="text-xs bg-white/10 text-muted-foreground border border-white/15 px-2.5 py-1 rounded-full font-medium">{phone.colors.split(',').length} Colors</span>
                  )}
                </div>
              </div>
            </div>
      ...
      {/* â•â•â• TAB CONTENT â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                ...
                <div className="glass-card rounded-2xl p-5">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {[
                      ...
                      { label: "Processor", value: phone.chipset?.split(' ').slice(0,3).join(' '), sub: phone.cpu?.split(' ').slice(0,4).join(' '), icon: Cpu, color: "#805AD5" },
                      ...
                      { label: "OS",        value: phone.os?.split(',')[0], sub: phone.chipset?.split(' ')[0],      icon: Shield,      color: "#C05621" },
                ...
                {(phone.main_camera || phone.selfie_camera) && (
                  <div className="glass-card rounded-2xl p-5">
                    ...
                    <div className="space-y-3">
                      {phone.main_camera && (
                        <div>
                          <p className="text-xs font-bold text-muted-foreground uppercase mb-1.5">Rear Camera</p>
                          <p className="text-sm text-foreground">{phone.main_camera}</p>
                        </div>
                      )}
                      {phone.selfie_camera && (
                        <div className="border-t border-white/10 pt-3">
                          <p className="text-xs font-bold text-muted-foreground uppercase mb-1.5">Front Camera</p>
                          <p className="text-sm text-foreground">{phone.selfie_camera}</p>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            {activeTab === "SPECS" && (
              <div>
                ...
                {specSections.length > 0 ? (
                  specSections.map((section, i) => <SpecSection key={i} {...section} />)
                ) : (
                  <div className="glass-card rounded-2xl p-10 text-center">
                    <p className="text-muted-foreground text-sm">Detailed specifications not available yet.</p>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="space-y-4">
            ...
            <div className="glass-card rounded-2xl overflow-hidden sticky top-32">
              ...
              <div className="p-5">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full py-3 rounded-xl font-bold text-sm text-white mb-2 hover:opacity-90 transition-all flex items-center justify-center gap-2 bg-primary"
                >
                  <ExternalLink size={16} />
                  Search Amazon
                </a>
                <a
                  href={`https://www.gsmarena.com/${phone.slug?.replace(/-/g, "_")}.php`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full glass-card py-3 rounded-xl font-bold text-sm hover:border-primary/50 transition-all flex items-center justify-center gap-2"
                >
            ...
            <div className="glass-card rounded-2xl p-4">
              ...
              >
                <ArrowLeft size={14} />
                Back to Phone Finder
              </Link>
            </div>

            {/* Info card */}
            <div className="glass-card rounded-2xl px-4 py-3 flex items-center justify-between">
              <span className="text-xs text-muted-foreground">{phone.brand} â€º {phone.name}</span>
            </div>
          </div>
        </div>
      </div>
